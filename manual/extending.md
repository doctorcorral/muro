...
- `Muro.Reduction` — parallel reduction `⇛` (δ, β, ι, `ann`, congruent under every constructor), closed under substitution, confluent by complete developments. Rigid heads are preserved along `⇛*`.
- `Muro.Convert` — the weak-head strategy `⟶` (exactly `Check.whnf`: deterministic, with normal and neutral forms `Nf` / `Ne`), and conversion `≈`, the equivalence generated by `⇛`. `≈` is a congruence, closed under substitution, and joinability by confluence; `≈-shape` separates rigid heads and `≈-pi-inj` / `≈-idt-inj` invert Π and ≡.
- `Muro.Judgement` — inductive ⊢ with uses for the core fragment (Π, λ, application, Nat, Unit, Empty, identity, `rewrite`, `def`, annotation). Conversion is `≈` in spec mode, so every def unfolds, as in `Check.whnf`.
- `Muro.Typing` — the declarative judgment `⊨` the metatheory is about: ⊢ without uses, conversion at the root of every rule. Proved: ⊢ is sound for ⊨ (`forget-⇐`), mode weakening along run ≤ evid ≤ spec, renaming, substitution, and preservation (`pres`: a `⟶` step keeps the type of a run- or evid-mode derivation, given `WfSig`).
- `Muro.Wall` — mode wall lemmas. No promotion.
- `Muro.Consistency` — no closed normal evidence term has type Empty (for ⊢ and for ⊨), closed evidence terms make progress, preservation holds at Empty; `Empty-evid` itself is not proved (normalisation of closed evidence is the one explicit hypothesis of `Empty-evid-from`), and data / ν / Tensor are still outside ⊢.
...
`make agda` is `agda --no-libraries -i agda -i vendor/agda-stdlib/src`, first with `--safe` on the theorem modules (`Muro.Wall`, `Muro.Consistency`, which import `Muro.Typing`, `Muro.Convert`, `Muro.Reduction`, `Muro.SubstLemmas`), then on the whole tree (`Muro.Check` and `Muro.Unembed` carry `TERMINATING` pragmas and are not `--safe`). The stdlib checkout ships extra `.agda-lib` files that must not be loaded.
