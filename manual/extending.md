...
4. **Judgement** (`agda/Muro/Judgement.agda`) if the constructor is in the core fragment: the same rule as an inductive constructor, with uses. If it reduces, a `⟶` rule in `agda/Muro/Convert.agda`. Then `Muro.Wall` and `Muro.Consistency` must still check under `--safe`; a new ⇒ rule needs a `spec-⇒-uses` clause and, if it is an introduction or eliminator, `Empty-intro` / `progress` clauses.
...
- `Muro.Judgement` — inductive ⊢ with uses for the core fragment (Π, λ, application, Nat, Unit, Empty, identity, `rewrite`, `def`, annotation). Conversion is the relation `≈` of `Muro.Convert` (the equivalence generated by δ/β/ι weak-head reduction, no fuel; `≈` is not congruent under binders).
- `Muro.Wall` — mode wall lemmas. No promotion.
- `Muro.Consistency` — no closed normal evidence term has type Empty, and closed evidence terms make progress; `Empty-evid` itself is not proved (preservation and normalisation are the explicit hypotheses of `Empty-evid-from`), and data / ν / Tensor are still outside ⊢.

`Muro.Judgement`, `Muro.Wall`, `Muro.Consistency`, and what they import are checked under `--safe`: no postulates. Example twins (`Example*.agda`) are examples of those rules.
...
`make agda` is `agda --no-libraries -i agda -i vendor/agda-stdlib/src`, first with `--safe` on the theorem modules (`Muro.Wall`, `Muro.Consistency`), then on the whole tree (`Muro.Check` and `Muro.Unembed` carry `TERMINATING` pragmas and are not `--safe`). The stdlib checkout ships extra `.agda-lib` files that must not be loaded.
