...
- `Muro.Spine` — application spines `appsFrom h as` as a relation (`Spine h as e`), read back by `unspine`, unique for rigid heads, and their renaming / substitution lemmas. The spine views `Check` uses (`ctorSpine`, `dtyArgs`, `defArgs`, `lamView`) are characterised here.
...
- `Muro.Soundness` — the executable checker is sound for ⊢ on the fragment. `whnf-sound` (`Check.whnf` is `⟶*` in spec and stays in the fragment), `conv-sound` (`Check.conv` says yes → `≈`), `isData-sound`, `instParams-sound`, and then `infer-sound` / `check-sound` / `checkTy-sound`: over a `GoodSig` (a `FragSig` whose data types are non-indexed and whose constructor types are telescopes ending in the data type) and a `FragCtx`, `infer k σ rs Γ m e ≡ ok (A , u)` gives a ⊢ derivation of `e` with uses `u` and a type convertible to `A`; `check` and `checkTy` likewise. `checkDef-sound` and `checkSig-sound` are the corollaries for definitions and whole signatures. The mutual proof is by structural recursion on a depth-indexed fragment predicate (`FragD`), because a constructor spine is taken apart by a function, not by pattern matching. This module imports `Muro.Check`, so it is not `--safe`.

`Muro.Judgement`, `Muro.Typing`, `Muro.Wall`, `Muro.Consistency`, `Muro.Frag`, `Muro.Tag`, and what they import are checked under `--safe`: no postulates. `Muro.Soundness` is checked without `--safe` because `Muro.Check` carries `TERMINATING` pragmas; it has no postulates and no pragmas of its own. Example twins (`Example*.agda`) are examples of those rules.
...
`make agda` is `agda --no-libraries -i agda -i vendor/agda-stdlib/src`, first with `--safe` on the theorem modules (`Muro.Wall`, `Muro.Consistency`, `Muro.Frag`, `Muro.Tag`, which import `Muro.Typing`, `Muro.Data`, `Muro.Convert`, `Muro.Reduction`, `Muro.Spine`, `Muro.SubstLemmas`), then on the whole tree (`Muro.Check` and `Muro.Unembed` carry `TERMINATING` pragmas and are not `--safe`; `Muro.Soundness` imports `Muro.Check`). `make agda-soundness` checks `Muro.Soundness` alone; it takes several minutes. The stdlib checkout ships extra `.agda-lib` files that must not be loaded.
